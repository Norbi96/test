                ...
                <table class="text-left w-full border-collapse">
                    <thead>
                        <tr>
                            ...
                            <th class="py-4 px-6 bg-grey-lightest font-bold uppercase text-sm text-grey-dark border-b border-grey-light">Blognév</th>
                    ...
                    <tbody>
                        @foreach ($blogies as $blog)
                            <tr class="hover:bg-grey-lighter">
                                ...
                                <td class="py-4 px-6 border-b border-grey-light">{{ $blog->name }}</td>
                                <td class="py-4 px-6 border-b border-grey-light">{{ $blog->content }}</td>
                                <td class="py-4 px-6 border-b border-grey-light">
                                    <form action="{{ route('blogies.destroy',$blog->id) }}" method="Post">
                                        <a href="{{ route('blogies.edit',$blog->id) }}"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg></a>
                                        @csrf
                                        @method('DELETE')
